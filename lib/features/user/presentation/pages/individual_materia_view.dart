...
//  - Status badge → banner full-width con días restantes
//  - Campos vacíos colapsados en una nota discreta
//  - Botones de acción anclados en bottomNavigationBar (sticky)
//  - Info del examen en header; info del profesor en card separada
//  - Email con acción "Copiar" (y mailto si se desea)
// ============================================================
...
class IndividualMateriaView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    debugPrint('MateriaData: nombre=${data.nombre}, profesor=${data.profesor}, email=${data.emailProfesor}, fecha=${data.fecha}, rawFecha=${data.rawFecha}, diasRestantes=${data.diasRestantes}, status=${data.status}');

    return Scaffold(
      backgroundColor: data.barColor,
      // ── Sticky bottom bar con las dos acciones ──────────────────────────
      bottomNavigationBar: _StickyActions(data: data),
      body: Stack(
        children: [
          // Fondo con patrón
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!data.tieneMaterial) _EmptyMaterialNote(),
                    ],
                  ),
                ),
              ),
...
// ---------------------------------------------------------------------------
// Sticky bottom bar con los dos botones de acción
// ---------------------------------------------------------------------------
...
class _StickyActionsState extends ConsumerState<_StickyActions> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          // Primario: agregar al calendario
          Expanded(
            child: isSaved
                ? OutlinedButton.icon(
